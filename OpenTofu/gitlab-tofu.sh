...
terraform_is_at_least() {
  [ "${1}" = "$(tofu -version | awk -v min="${1}" '/^OpenTofu v/{ sub(/^v/, "", $2); print min; print $2 }' | sort -V | head -n1)" ]
  return $?
}
...
# Expose Gitlab specific variables to tofu since no -tf-var is available
...
# Use tofu automation mode (will remove some verbose unneeded messages)
export TF_IN_AUTOMATION=true

DEFAULT_TF_CONFIG_PATH="$HOME/.terraformrc"

# Set a tofu CLI Configuration File
...
if [ $sourced -eq 0 ]; then
  ...
  case "${1}" in
    ...
    ;;
    "init")
      # shift argument list „one to the left“ to not call 'tofu init init'
      shift
      terraform_init "${@}"
    ;;
    "plan")
